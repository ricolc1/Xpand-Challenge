...
class ActorControllerTests {
	...
	void doTestCreateActor() {
		HttpEntity<ActorDTO> postRequest = new HttpEntity<>(mockActor);
		ResponseEntity<?> postResponse = restTemplate.postForEntity(path(port), postRequest, Void.class);
	...
	@Test
	void doTestAssignActor() {
		ResponseEntity<IdentifiableActorDTO> response = restTemplate.getForEntity(path(port) + "/1", IdentifiableActorDTO.class);
		ResponseEntity<?> putResponse = restTemplate.postForEntity(path(port) + "/" + response.getBody().getId() + "/1", null, Void.class);
		assertEquals(HttpStatus.NO_CONTENT.value(), putResponse.getStatusCode().value());
		ResponseEntity<IdentifiableMovieDTO> movieResponse = restTemplate.getForEntity("http://localhost:"+port+"/movies/1", IdentifiableMovieDTO.class);
		assertEquals(movieResponse.getBody().getActors().get(0).getId(), response.getBody().getId());
	}
